...
// ahí sin condicional (igual que sendVitalWhatsApp reemplazó a sendSignatureWhatsApp).
async function sendVitalSignatureSms({ clientName, clientPhone, token }) {
  const to = `+${normalizePhone(clientPhone)}`;
  const body = `Estimado(a) ${clientName}, desde Vital Health Insurance le hacemos llegar el documento de autorización para el tratamiento de sus datos personales. Fírmelo aquí (válido 72h, un solo uso): ${buildSigningUrl(token)}`;
