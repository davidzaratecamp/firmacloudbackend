...
async function sendCartaFormulario({ clientName, clientEmail, token, requestId, cartaPath, npnName, insurerName }) {
  ...
  const brand         = insurerName || 'Ambetter Health';
...
<table width="620" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 16px rgba(0,0,0,.10);">
  ...
  <tr>
    <td style="background:linear-gradient(135deg,#1e3a5f 0%,#2563eb 100%);padding:20px 32px;">
      <p style="margin:0;color:#93c5fd;font-size:11px;letter-spacing:1.5px;text-transform:uppercase;">${brand}</p>
  ...
  <tr>
    <td style="padding:28px 32px 0;">
      ...
      <p style="margin:0 0 14px;font-size:14px;color:#374151;line-height:1.8;">
        Nos comunicamos con usted de parte de <strong>${brand}</strong> para informarle que,
  ...
  <tr>
    <td style="background:#f9fafb;padding:14px 32px;text-align:center;border-top:1px solid #e5e7eb;">
      <p style="font-size:11px;color:#9ca3af;margin:0;">
        Este mensaje fue enviado por <strong>Asiste Health Care</strong> en nombre de <strong>${brand}</strong> &middot;
  ...
  const text = [
    ...
    '',
    `Nos comunicamos con usted de parte de ${brand} para informarle que es necesario actualizar su información personal de manera urgente.`,
    '',
  ...
  await transporter.sendMail({
    ...
    to:      clientEmail,
    subject: `URGENTE: Actualización de datos requerida — ${brand}`,
    text,
    html,
